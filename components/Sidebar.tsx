...
export function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  const favoriteTools = useMemo(() => {
    return filteredTools.filter((t) => isFavorite(t.id));
  }, [filteredTools, isFavorite]);
    ...
    <div className="flex flex-col h-full bg-[#0b1224] border-r border-white/10">
      ...
      <nav className="flex-1 overflow-y-auto px-4 pb-6 space-y-6 custom-scrollbar">
        {favoriteTools.length > 0 && (
          <div className="mb-6">
            <h3 className="flex items-center gap-2 text-xs font-semibold text-brand uppercase tracking-wider mb-3 px-2">
